...
//			$output = $this->CI->load->view('profiler_template', array('sections' => $this->_sections), true);
                        ...
                        foreach ($this->_sections['queries'][count($this->_sections['queries'])-1] as $key => $val){
                            $query = $key;
                        }

                        $bencmark_str = '';

                        foreach ($this->_sections['benchmarks'] as $key => $val) {
                            $key_name = substr($key, 0, 12);

                            switch ($key_name){
                                case 'Loading Time':
                                    $_POST['base_classes'] = $val;
                                    break;
                                case 'Controller E':
                                    $method_name = explode('Controller Execution Time (', $key);
                                    $method_name = explode(')', $method_name[1]);
                                    $method_name = $method_name[0];
                                    $_POST['method_name'] = $method_name;
                                    $_POST['controller'] = $val;
                                    break;
                                case 'Total Execut':
                                    $_POST['total'] = $val;
                                    break;
                                default :
                                    $bencmark_str = $bencmark_str.' '.$key.': '.$val.'s';
                                    break;
                            }
                        }

                        $_POST['date_time'] = date('Y-m-d'). ' ' . date('H:i:s');
                        $_POST['bencmarks'] = $bencmark_str;
                        $_POST['query_time'] = $query ? $query : '';
                        $_POST['query_quant'] = count($this->_sections['queries']) ? count($this->_sections['queries']) : 0;
                        $_POST['memory_usage'] = round(memory_get_usage()/1024/1024, 2);
                        $_POST['files'] = count($this->_sections['files']);

//                        $this->load->model('productivity');
                        if ($method_name !== ' User Controller / Get Indexes Online '){
                            $productivity = new Productivity();
                            $productivity->set_productivity();
                        }
//
//                        $output_string = $bencmark_str . ' MU: ' . round(memory_get_usage()/1024/1024, 2) . ' MB DBQ: ' . count($this->_sections['queries']) . ' TDBQ:' . $query . 's F: ' . count($this->_sections['files']);
//                        log_message('error', $output_string);
		}
		else
		{
