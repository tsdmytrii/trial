	...
	Construct.prototype = {
		...
		init: function()
		{
			...
			$(this.doc).keydown(function(e)
			{
				...
				if (e.keyCode == 9) {
					this.execCommand('indent', false);
					return false;
				}
				if (e.keyCode == 66 && isCtrl) {
					this.execCommand('bold', 'bold');
					return false;
				}
				if (e.keyCode == 73 && isCtrl) {
					this.execCommand('italic', 'italic');
					return false;
				}			        
		...
		focus: function()
		{
			if ($.browser.msie) $(this.frame).load(function() {
		...
		purify: function(html){
			...
			function preProcess(html) {
				console.log('Before preprocess:' + html);
				var each = $.each;
				//var ed = this.editor,
				var	h = html,
				o = {},
				...
				if (/class="?Mso|style="[^"]*\bmso-|w:WordDocument/i.test(h) || o.wordContent) {
					//alert('word content detected');
					o.wordContent = true;			// Mark the pasted contents as word specific content
					//console.log('Word contents detected.');
					//	alert('enter this case');
		...
		enableAir: function()
		{
			...
			{	
				...
				$(this.doc).bind('textselect', this.frameID, function(e)
				{
					...
					$('#imp_redactor_air_' + this.frameID).css({
						left: e.clientX + 'px', 
						top: (e.clientY + 8) + 'px'
					}).show();
		...
		docObserve: function()
		{
			...
			body.find('img').click(function(e) {
				this.imageEdit(e);
			}.bind2(this));
			body.find('table').click(function(e) {
				this.tableObserver(e);
			}.bind2(this));
			body.find('.redactor_file_link').click(function(e) {
				this.fileEdit(e);
			}.bind2(this));
		...
		imageEdit: function(e)
		{
			var handler = function()
			{
				...
				$('#redactor_image_width').val($el.width());

			}.bind2(this);       
        
			redactorActive = this;      
			this.modalInit(RLANG.image, this.opts.path + 'plugins/image_edit.html', 380, 320, handler);
		...
		imageSave: function(el)
		{
			...
			var width = $('#redactor_image_width').val();
			//		var height = $('#redactor_image_height').val();
			//alert(parseInt(width));
			if(width == parseInt($('#redactor_image_width').val())){
				$(el).css('width', width);
			}
			//	if(height== parseInt($('#redactor_image_height').val())){
			...
			if ($('#redactor_form_image_align') != 0)
			{
				...
				if (float == 'left') $(el).css({
					float: 'left', 
					margin: '10px'
				});
				else if (float == 'right') $(el).css({
					float: 'right', 
					margin: '10px'
				});
			}
		...
		modalInit: function(title, url, width, height, handler, scroll)
		{
			if (this.opts.overlay) 
			{
				...
				$('#redactor_imp_modal_overlay').click(function() {
					this.modalClose();
				}.bind2(this));
			}
			...
			$('#redactor_imp_modal_close').click(function() {
				this.modalClose();
			}.bind2(this));
		...
		modalClose: function()
		{
			...
			$('#redactor_imp_modal_close').unbind('click', function() {
				this.modalClose();
			}.bind2(this));
			$('#redactor_imp_modal').fadeOut('fast', function()
			{
				...
				if (this.opts.overlay) 
				{
					...
					$('#redactor_imp_modal_overlay').unbind('click', function() {
						this.modalClose();
					}.bind2(this));					
				}			
				...
				function (i, s)
				{
					...
					else
					{
						...
						if (typeof(s.func) == 'undefined') a.click(function() {
							this.execCommand(s.exec, s.name);
						}.bind2(this));
							...
							$.each(s.dropdown,
								function (x, d)
								{
									...
									else
									{
										...
										if (typeof(d.func) == 'undefined') $(ul_li_a).click(function() {
											this.execCommand(d.exec, d.name);
										}.bind2(this));
										else $(ul_li_a).click(function(e) {
											this[d.func](e);
										}.bind2(this));										
									}
						...
						if (s.name == 'backcolor' || s.name == 'fontcolor' || typeof(s.dropdown) != 'undefined')
						{
							...
							this.hdlHideDropDown = function(e) {
								this.hideDropDown(e, ul, s.name)
							}.bind2(this);
							this.hdlShowDropDown = function(e) {
								this.showDropDown(e, ul, s.name)
							}.bind2(this);
							this.hdlShowerDropDown = function(e) {
								this.showerDropDown(e, ul, s.name)
							}.bind2(this);   	
