...
jQuery.extend({
    ...
    newWindow: function(options) {
        ...
        $winBtn = $('<div class="window-buttons"></div>');
        ...
        if (options.minimizeButton) $winBtn.append($windowMinimizeButton);
        if (options.maximizeButton) $winBtn.append($windowMaximizeButton);
        if (options.closeButton) $winBtn.append($windowCloseButton);
        $titleBar.append($winBtn);
        ...
        var resize = function($obj, width, height) {
            ...
            if (options.type === "iframe") {
                ...
                $obj.find("iframe").each(function(){
                    ...
                    position: "absolute", opacity: "0.0001", zIndex: 100,
                    background:"#444"});
                });
            }
        }
        ...
        var dragging = function(e, $obj) {
            if (options.draggable && $obj.data('draggable') == true) {
                e = e ? e : window.event;
...
//            h = h < 400 ? 400 : h;
            ...
            neww = neww < 600 ? 600 : neww;
            newh = newh < 400 ? 400 : newh;
            lastMouseX = e.clientX;
            lastMouseY = e.clientY;

            resize($obj, neww, newh);

            var nconth = newh - $obj.find('.window-statusBar').height() - $obj.find('.window-titleBar').height() + 2;

            $obj.find('.window-content').css('height', nconth);

            $obj.find('.jspTrack').css({
                height: '100%'
            });
        };

        //Attach a mousedown event to the window's title bar to allow dragging
        $titleBarContent.on('mousedown.jquerywindow', function(e) {
...
//            setFocus($obj);
            ...
            if ($obj.data("state") != "maximized") {
                ...
                $(document).on('mousemove.jquerywindow', function(e) {
                    dragging(e, $obj);
                });

                $(document).on('mouseup.jquerywindow', function(e) {
...
//        });
        ...
        $windowResizeIcon.on('mousedown.jquerywindow', function(e) {
            $obj = $(e.target).parent().parent();
//            setFocus($obj);
            disableSelection();
...
//        $windowMinimizeButton.on('click.jquerywindow', function(e) {
//            $obj = $(e.target).parent().parent();
//            setFocus($obj);
...
//                    $obj.find(".window-statusBar").css("display", 'block');
//                });
//            }
//            else if ($obj.data("state") === "normal") {
//                $obj.data("state", "minimized");
//                $obj.find(".window-statusBar").css("display", 'none');
...
//        $windowMaximizeButton.on('click.jquerywindow', function(e) {
//            $obj = $(e.target).parent().parent();
//            setFocus($obj);
//
...
//        $windowCloseButton.on('click.jquerywindow', function(e) {
...
//        });
        ...
        $modalBackground.on('click.jquerywindow', function(e) {
